...
parameters {auto merr : MErr m}
  ...
  ||| Handles an error of a single type by running it through
  ||| the given effectful computation.
  export
  handleError : (0 e : Type) -> Has e es => (e -> m es a) -> m es a -> m es a
  handleError e f =
  ...
  export
  catch : (0 e : Type) -> Has e es => m es a -> m es (Either e a)
  catch e m =
    attempt m >>= \case
  ...
  export
  extractErr : (0 e : Type) -> (p : Has e es) => m es a -> m (es - e) (Either e a)
  extractErr e m =
  ...
  export %inline
  mapErrors : (HSum es -> HSum fs) -> m es a -> m fs a
  mapErrors f = handleErrors (fail . f)

  export %inline
  mapErr : (h : Has e es) => (e -> f) -> m es a -> m (Replaced es h f) a
  ...
  export %inline
  widenErrors : All (`Elem` es) fs => m fs a -> m es a
  widenErrors = mapErrors widen_

  export %inline
  weakenErrors : m [] a -> m fs a
  ...
  export %inline
  liftError : m [e] a -> m fs (Either e a)
  liftError = handleErrors (pure . Left . project1) . map Right

  export %inline
  handle : All (\e => e -> m [] a) es -> m es a -> m [] a
  ...
  export %inline
  onError : m es a -> (HSum es -> m [] ()) -> m es a
  ...
  export
  ifError : Has e es => Eq e => (err : e) -> Lazy a -> m es a -> m es a
  ifError err v =
    handleErrors $ \x => case project e x of
      Nothing => fail x
